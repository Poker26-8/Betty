...
Public Class frmVentas1
    ...
    'ESTOS VIENEN DEL BOTON DE VENTAS
    ...
    Public Sub leePeso()
        ...
        Try
            Dim cnn100 As MySqlConnection = New MySqlConnection(sTargetlocal)
    ...
    Private Async Sub frmVentas1_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            cmd1 = cnn1.CreateCommand
            cmd1.CommandText =
                    "select Impresora from RutasImpresion where Equipo='" & ObtenerNombreEquipo() & "' and Tipo='" & cboimpresion.Text & "'"
            rd1 = cmd1.ExecuteReader
            If rd1.HasRows Then
                If rd1.Read Then
    ...
    Private Sub txtcontraseña_KeyPress(sender As Object, e As System.Windows.Forms.KeyPressEventArgs) Handles txtcontraseña.KeyPress
        If AscW(e.KeyChar) = Keys.Enter Then
            ...
            Try
                ...
                cmd1 = cnn1.CreateCommand
                cmd1.CommandText = "select Vent_NVen from Permisos where IdEmpleado= " & VarIdUsuario
                rd1 = cmd1.ExecuteReader
                If rd1.HasRows Then
                    If rd1.Read Then
...
#End Region
    ...
    Public Sub buscadetalledevo()
        ...
        Dim sql69 As String = "Select * from ventasdetalle inner JOIN productos  ON ventasdetalle.codigo=productos.codigo  inner join tb_moneda on  productos.id_tbmoneda=tb_moneda.Id where Folio=" & cbonota.Text & " order by ventasdetalle.Nombre"
        Dim dr69 As DataRow
        With odata69
    ...
    Private Sub txtcantidad_TextChanged(sender As Object, e As System.EventArgs) Handles txtcantidad.TextChanged
        ...
        Try
            ...
            If cbotipo.Visible = False Then
                ...
                Next
                If index1 <> -1 Then
                    TiCambio = IIf(DataGridView1.Rows(index1).Cells("tipo_cambio").Value.ToString() = "0", ",", DataGridView1.Rows(index1).Cells("tipo_cambio").Value.ToString())
                    If T_Precio = "DIA_NOCHE" And (H_Actual > H_Inicia Or H_Actual < H_Final) Then
            ...
            If rd1.HasRows Then
                If rd1.Read Then
                    If CStr(rd1(0).ToString) = "SERVICIOS" Then
                        If Edita = False And AscW(e.KeyChar) <> 13 Then e.KeyChar = Nothing
                        If AscW(e.KeyChar) = Keys.Enter Then
                            cnn2.Close() : cnn2.Open()
                            cmd2 = cnn2.CreateCommand
                            cmd2.CommandText =
...
kakaxd:
        ...
        Dim insertaventalocal As String = ""
        Try
            Select Case lblNumCliente.Text
                Case Is = "MOSTRADOR"
                    ...
                    If CDbl(txtdescuento1.Text) > 0 Then
                        ...
                        insertaventalocal = "insert into Ventas2(IdCliente,Cliente,Direccion,Subtotal,IVA,Totales,Descuento,Devolucion,ACuenta,Resta,Usuario,FVenta,HVenta,FPago,FCancelado,Status,Comisionista,Comision,Concepto,MontoSinDesc,FEntrega,Entrega,Comentario,StatusE,FolMonedero,CodFactura,IP,Formato,Pedido,Fecha,FolioServidor,NumSuc) values(" & IdCliente & ",'" & IIf(cboNombre.Text = "", "PUBLICO EN GENERAL", cboNombre.Text) & "','" & nuvdire & "'," & SubTotal & "," & IVA_Vent & "," & Total_Ve & "," & sumadescuento & ",0," & ACuenta & "," & Resta & ",'" & lblusuario.Text & "','" & Format(Date.Now, "yyyy-MM-dd") & "','" & Format(Date.Now, "yyyy-MM-dd HH:mm:ss") & "','" & fecha_pago & "','','" & MyStatus & "','" & cbocomisionista.Text & "'," & totalcomision & ",''," & MontoSDesc & ",'" & Format(dtpFecha_E.Value, "dd/MM/yyyy") & "',0,'" & observaciones & "',0,'" & txttel.Text & "','" & cadenafact & "','" & dameIP2() & "','" & cboimpresion.Text & "'," & IIf(lblpedido.Text = "", 0, lblpedido.Text) & ",'" & Format(Date.Now, "yyyy-MM-dd HH:mm:ss") & "','FOLIOSERVIDOR1'," & base & ")"
                        ...
                        Dim dr12 As DataRow
                        With odata12
                            If .dbOpen(cnn12, sTarget, sinfo12) Then
        ...
        Do Until MYFOLIO <> 0
            ...
            With odata12
                If .dbOpen(cnn12, sTarget, sinfo12) Then
                    If .getDr(cnn12, dr12, "select MAX(Folio) from Ventas where IP='" & dameIP2() & "'", sinfo12) Then
                        MYFOLIO = dr12(0).ToString()
                    End If
                    cnn12.Close()
                End If
            End With
        ...
        'HACER EL INSERT DE VENTAS LOCAL
        Dim cnnA As OleDb.OleDbConnection = New OleDb.OleDbConnection
        Dim sinfoA As String = ""
        Dim odataA As New ToolKitSQL.oledbdata

        With odataA
            If .dbOpen(cnnA, Direcc_Access, sinfoA) Then
                Dim insertaventalocal_M As String = ""
                insertaventalocal_M = Replace(insertaventalocal, "FOLIOSERVIDOR1", MYFOLIO)
                .runSp(cnnA, insertaventalocal_M, sinfoA)
...
Door:
                If grdcaptura.Rows(R).Cells(0).Value.ToString = "" And grdcaptura.Rows(R).Cells(1).Value.ToString <> "" Then
                    Dim comentario As String = grdcaptura.Rows(R).Cells(1).Value.ToString.Replace(vbCrLf, "").Replace(vbCr, "").Replace(vbLf, "")

                    Dim sqlcomentario As String = ""
                    sqlcomentario = "update VentasDetalle2 set Comentario='" & comentario & "' where Codigo='" & mycode & "' and Folio=" & MYFOLIO


                    Dim cnn120 As OleDb.OleDbConnection = New OleDb.OleDbConnection
                    Dim sinfo120 As String = ""
                    Dim odata120 As New ToolKitSQL.oledbdata
                    With odata120
                        If .dbOpen(cnn120, Direcc_Access, sinfo120) Then
                ...
                If grdcaptura.Rows(R).Cells(15).Value() <> "" Then
                    ...
                    Dim creainsert As String = ""
                    creainsert = "insert into VentasDetalle2(Folio,Codigo,Nombre,Unidad,Cantidad,CostoVP,CostoVUE,Precio,Total,PrecioSinIVA,TotalSinIVA,Fecha,FechaCompleta,Comisionista,Facturado,Depto,Grupo,CostVR,Descto,VDCosteo,TotalIEPS,TasaIEPS,Caducidad,Lote,CantidadE,Promo_Monedero,Unico,Descuento,Gprint,CodUnico,NumSuc) values(" & MYFOLIO & ",'" & mycode & "','" & mydesc & "','" & myunid & "'," & mycant & "," & MyProm & "," & MyCostVUE & "," & mypreciodescuento & "," & mytotal & "," & myprecioS & "," & mytotalS & ",'" & Format(Date.Now, "yyyy-MM-dd") & "','" & Format(Date.Now, "yyyy-MM-dd HH:mm:ss") & "','" & cbocomisionista.Text & "','0','" & MyDepto & "','" & MyGrupo & "','0'," & descuentoproducto & ",0," & ieps & "," & tasaieps & ",'" & caduca & "','" & lote & "',0," & monedero & "," & IIf(Unico = False, 0, 1) & "," & descuentoproducto & ",'" & gprint & "','" & codunico & "'," & base & ")"
                    ...
                    With odata27
                        If .dbOpen(cnn27, Direcc_Access, sinfo27) Then
                            If .runSp(cnn27, creainsert, sinfo27) Then
                            Else
                                MsgBox(sinfo27)
                            End If
                            cnn27.Close()
                    ...
                    '        cmd1 = cnn1.CreateCommand
                    '        cmd1.CommandText =
                    '            "insert into Cardex(Codigo,Nombre,Movimiento,Inicial,Cantidad,Final,Precio,Fecha,Usuario,Folio,Tipo,Cedula,Receta,Medico,Domicilio) values('" & mycodd & "','" & mydesc & "','Venta'," & Existencia & "," & mycant & "," & MyExiste * MyMulti2 & "," & myprecio & ",'" & Format(Date.Now, "yyyy-MM-dd HH:mm:ss") & "','" & lblusuario.Text & "','" & MYFOLIO & "','','','','','')"
                    '        cmd1.ExecuteNonQuery()
                    '    End If
                    'End If
            ...
            Try
                ...
                If Resta_ventas < Total_devo Then
                    ...
                    If lblNumCliente.Text <> "MOSTRADOR" Then
                        ...
                        cmd1 = cnn1.CreateCommand
                        cmd1.CommandText =
                            "insert into AbonoI(NumFolio,IdCliente,Cliente,Concepto,Fecha,Hora,FechaCompleta,Cargo,Abono,Saldo,FormaPago,Monto,Banco,Referencia,Usuario,Corte,CorteU,Cargado,MontoSF) values(" & cbonota.Text & "," & lblNumCliente.Text & ",'" & cboNombre.Text & "','CARGO','" & Format(Date.Now, "yyyy-MM-dd") & "','" & Format(Date.Now, "HH:mm:ss") & "','" & Format(Date.Now, "yyyy-MM-dd HH:mm:ss") & "'," & Total_devo & ",''," & (MySaldo - Resta_ventas) & ",'EFECTIVO',0,'','','" & lblusuario.Text & "',0,0,0,0)"
                        cmd1.ExecuteNonQuery()
                    End If
...
ecomoda:
    ...
    Private Sub pVenta80_PrintPage(sender As System.Object, e As System.Drawing.Printing.PrintPageEventArgs) Handles pVenta80.PrintPage
        ...
        Try
                    ...
                    If CStr(grdcaptura.Rows(N).Cells(0).Value.ToString) <> "" Then
                        ...
                        If index <> -1 Then
                            ...
                            If DataGridView1.Rows(index).Cells("IVA").Value.ToString() > 0 Then
                                ...
                                IVAPRODUCTO = MySubtotal / (1 + DataGridView1.Rows(index).Cells("IVA").Value.ToString())
            ...
            e.Graphics.DrawString("Total:", fuente_prods, Brushes.Black, 40, Y)
            e.Graphics.DrawString(simbolo & FormatNumber(txtPagar.Text, 2), fuente_prods, Brushes.Black, 270, Y, sf)
            Y += 18

            e.Graphics.DrawString(convLetras(txtPagar.Text), New Drawing.Font(tipografia, 7, FontStyle.Italic), Brushes.Black, 40, Y)
            Y += 18

            If CDbl(txtefectivo.Text) > 0 Then
                e.Graphics.DrawString("Efectivo:", fuente_prods, Brushes.Black, 40, Y)
                e.Graphics.DrawString(simbolo & FormatNumber(txtefectivo.Text, 2), fuente_prods, Brushes.Black, 270, Y, sf)
                Y += 20
            End If
            If CDbl(txtCambio.Text) > 0 Then
                e.Graphics.DrawString("Cambio:", fuente_prods, Brushes.Black, 40, Y)
...
doorcita:
    ...
    Private Sub cbodesc_DropDown_1(sender As Object, e As EventArgs) Handles cbodesc.DropDown
        If Serchi = True Then
            Serchi = False
        Else
        End If
    ...
    Private Sub cbodesc_KeyDown_1(sender As Object, e As KeyEventArgs) Handles cbodesc.KeyDown
        Select Case e.KeyCode
            Case Is = Keys.F2
                If cbodesc.Text <> "" Then MsgBox("Primero baja el producto para asignar un comentario.", vbInformation + vbOKOnly, "Delsscom Control Negocios Pro") : cbodesc.Focus().Equals(True) : Exit Sub
                If grdcaptura.Rows.Count = 0 Then MsgBox("No cuentas con productos para asignar comentarios.", vbInformation + vbOKOnly, "Delsscom Control Negocios Pro") : Exit Sub
